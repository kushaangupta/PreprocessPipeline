...
def create_channel_map(
    ...
    anat_grps, spk_channels, has_spk_groups, skipped_channels, root = load_xml_metadata(
        xml_path
    )
    ...
    for probe_idx, probe in enumerate(probe_assignments):
        ...
        print(
            f"Processing Probe {probe_idx+1}: Type={p_type}, Groups={p_groups}, X_Offset={p_x_offset}"
        )
        ...
        for local_idx, g_idx in enumerate(p_groups):
            ...
            if p_type == "double_sided":
                ...
                shank_pitch = 200  # um between shanks
                x[:] = horizontal_spacing
                y = np.arange(n_ch) * -vertical_spacing
                x[::2] = -horizontal_spacing
                x = x + shank_id * shank_pitch

            elif p_type == "poly3":
                vertical_spacing = 30
                horizontal_spacing = 18.5
                shank_pitch = 250  # um between shanks
                ext = n_ch % 3  # channels to extend for even distribution
                poly = np.arange(n_ch - ext) % 3
                x[:] = 0
                idx_l = np.where(poly == 1)[0]
                idx_c = np.where(poly == 0)[0]
                idx_r = np.where(poly == 2)[0]
                x[idx_l] = -horizontal_spacing
                x[idx_c] = 0
                x[idx_r] = horizontal_spacing
                x[-ext:] = 0
                mask_r = x == horizontal_spacing
                y[mask_r] = np.arange(np.sum(mask_r)) * -vertical_spacing - (
                    0.5 * vertical_spacing
                )
                mask_0 = x == 0
                y[mask_0] = np.arange(np.sum(mask_0)) * -vertical_spacing
                mask_l = x == -horizontal_spacing
                y[mask_l] = np.arange(np.sum(mask_l)) * -vertical_spacing - (
                    0.5 * vertical_spacing
                )
                x = x + shank_id * shank_pitch
...
def convert_dual_side_map(
    chan_map_file, x_shift=6.0, pairs_to_merge=None, custom_shank_positions=None
):
    """
    ...
    for s_back, s_front in pairs_to_merge:
        ...
        print(
            f"Merged Shank {s_front} into {s_back}. Shifted X by {-current_offset + x_shift:.2f} um."
        )
    ...
    if custom_shank_positions:
        ...
        for s_id, target_x in custom_shank_positions.items():
            ...
            if np.any(idx_group):
                ...
                print(
                    f"  Shank {s_id}: Moved to {target_x:.1f} (shift {target_x - start_mean:.1f})"
                )
            ...
            if isinstance(data, np.ndarray):
                ...
                if data.ndim > 1:
                    ...
                    data = np.ascontiguousarray(
                        np.transpose(data, axes=range(data.ndim - 1, -1, -1))
                    )
...
def rezToPhy(rez, save_path):
    ...
    np.save(
        os.path.join(save_path, "spike_times.npy"), np.ascontiguousarray(spike_times)
    )
    np.save(
        os.path.join(save_path, "spike_templates.npy"),
        np.ascontiguousarray(spike_templates),
    )
    np.save(
        os.path.join(save_path, "spike_clusters.npy"),
        np.ascontiguousarray(spike_clusters),
    )

    np.save(os.path.join(save_path, "amplitudes.npy"), np.ascontiguousarray(amplitudes))
    np.save(
        os.path.join(save_path, "templates.npy"),
        np.ascontiguousarray(templates.astype(np.float32)),
    )
    np.save(
        os.path.join(save_path, "templates_ind.npy"),
        np.ascontiguousarray(templates_inds),
    )

    np.save(
        os.path.join(save_path, "channel_map.npy"),
        np.ascontiguousarray(chan_map_0ind[connected]),
    )
    coords = np.column_stack((xcoords[connected], ycoords[connected]))
    np.save(
        os.path.join(save_path, "channel_positions.npy"), np.ascontiguousarray(coords)
    )

    np.save(
        os.path.join(save_path, "template_features.npy"),
        np.ascontiguousarray(template_features.astype(np.float32)),
    )
    np.save(
        os.path.join(save_path, "template_feature_ind.npy"),
        np.ascontiguousarray(template_feature_inds),
    )
    np.save(
        os.path.join(save_path, "pc_features.npy"),
        np.ascontiguousarray(pc_features.astype(np.float32)),
    )
    np.save(
        os.path.join(save_path, "pc_feature_ind.npy"),
        np.ascontiguousarray(pc_feature_inds),
    )

    whitening_matrix = rez["Wrot"] / 200
    whitening_matrix_inv = np.linalg.pinv(
        whitening_matrix
    )  # Use pinv for numerical stability
    np.save(
        os.path.join(save_path, "whitening_mat.npy"),
        np.ascontiguousarray(whitening_matrix.astype(np.float32)),
    )
    np.save(
        os.path.join(save_path, "whitening_mat_inv.npy"),
        np.ascontiguousarray(whitening_matrix_inv.astype(np.float32)),
    )

    if "simScore" in rez:
        np.save(
            os.path.join(save_path, "similar_templates.npy"),
            np.ascontiguousarray(rez["simScore"].astype(np.float32)),
        )
